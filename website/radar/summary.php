...
if(is_dir($dir))foreach(glob($dir.DIRECTORY_SEPARATOR.'events-*.jsonl') ?: [] as $file){
 ...
 while(($line=fgets($fh))!==false){
  ...
  inc($dims['client_kind'],$kind);if($agent!=='')inc($dims['agent_label'],$agent);
...
foreach($byDay as &$r){foreach($r['visitors'] as $v=>$_)if(isset($returning[$v]))$r['returning'][$v]=true;$r['sessions']=count($r['sessions']);$r['visitors']=count($r['visitors']);$r['interested']=count($r['interested']);$r['returning']=count($r['returning']);$r['contacts']=count($r['contacts']);}unset($r);ksort($byDay);foreach($dims as &$q)arsort($q);unset($q);
