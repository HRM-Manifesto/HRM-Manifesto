...
checks={'payload_hashes':not bad,'unique_unit_ids':len({u['id'] for u in units})==len(units),'charter_46_each':all(counts[('charter',l)]==46 for l in ('pl','en','sv')),'decalogue_10_each':all(counts[('decalogue',l)]==10 for l in ('pl','en','sv')),'concepts_12':len(concepts)==12,'website_gateway_complete':all((REPO/'website'/'ai'/'1.0.0'/x).exists() for x in links),'unsigned_explicit':rel['signature_status']=='unsigned_pending_founder_signature'}
