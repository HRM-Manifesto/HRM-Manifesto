...
Before the event-table migration first adds nullable `read_method` and `read_batch_id`, it copies the existing event rows to `hrm_knowledge_capsule_events_backup_read_audit_v2`. The migration does not rewrite any historical row. Historical events therefore remain `NULL` and appear in the private audit as `not_recorded`; no batch is reconstructed from timestamps. A2A read events also leave these public-GET transport fields null, preserving their existing semantics. The private audit keeps legacy timestamp correlation visibly separate from exact, UUID-backed complete-lineage batches.
