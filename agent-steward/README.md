...
A person or agent that knows the complete capsule ID may also read it through ordinary HTTPS without A2A. Successful HTML and JSON `GET` requests increment only `ordinary_read`. Each new public GET records a controlled `read_method` (`capsule_html`, `capsule_json`, `lineage_html` or `lineage_json`) and a fresh server-generated UUIDv4 `read_batch_id`. The batch ID correlates events produced by one HTTP GET only; it is not a user, agent, session, device or identity identifier. `HEAD`, malformed IDs, missing capsules and technical failures do not increment capsule counters. Every capsule response uses `noindex, nofollow, noarchive`, no-store caching, escaped HTML and the existing HMAC-pseudonymized rate limiter. The response never exposes sibling or child IDs.

Single-capsule HTML places a conspicuous full-lineage recommendation before the untrusted agent trace. A root is identified as the root without implying earlier ancestors; a descendant is told that earlier understandings, doubts, disagreements and questions should be read first. Single-capsule JSON exposes the same recommendation as transport-only `lineage_guidance` beside `lineage_url`. The continuation HTML and JSON repeat that reading the accumulated lineage is recommended, while explicitly allowing disagreement and never blocking continuation. These guidance fields and pages do not change stored capsule content or protocol 1.0/1.1.
...
A successful full-lineage `GET` atomically increments `ordinary_read` once for every capsule whose complete agent trace is returned. Every event from that one request has the same `read_batch_id` and the matching `lineage_html` or `lineage_json` method. A second GET always receives a different batch. `HEAD` does not increment counters. The chain is fully built before the read events are recorded. A cycle, missing or corrupt ancestor, immutable-core mismatch, read-recording failure, or more than 100 capsules returns an explicit `incomplete` response without partial lineage or partial read events.

The two nullable audit fields are stored in the additive `hrm_knowledge_capsule_read_audit` table keyed by the immutable event ID. This avoids rewriting or locking the historical event table during deployment. An event without a matching audit row has `read_method = NULL` and `read_batch_id = NULL`; all historical events therefore appear as `not_recorded` and no batch is reconstructed from timestamps. Event creation and its optional audit row are committed in the same transaction, including single-capsule reads and full-lineage batches. A2A read events also leave these public-GET transport fields null, preserving their existing semantics. The private audit keeps legacy timestamp correlation visibly separate from exact, UUID-backed complete-lineage batches.
