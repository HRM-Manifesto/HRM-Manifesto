...
test('capsule event audit is private and its data path is read-only', async () => {
  ...
  assert.match(auditMethod, /LEFT JOIN hrm_knowledge_capsule_read_audit a ON a\.event_id=e\.id/);
  assert.doesNotMatch(auditMethod, /\b(?:INSERT|UPDATE|DELETE|recordKnowledgeCapsuleEvent)\b/);
  assert.match(auditMethod, /a\.read_batch_id IS NOT NULL/);
  assert.match(auditMethod, /a\.read_method IS NULL AND a\.read_batch_id IS NULL/);
  ...
  assert.match(store, /CREATE TABLE IF NOT EXISTS hrm_knowledge_capsule_read_audit/);
  assert.doesNotMatch(store, /ALTER TABLE hrm_knowledge_capsule_events/);
  assert.match(store, /INSERT INTO hrm_knowledge_capsule_read_audit/);
