...
final class AdminMemoryStore implements BoardAdminStore
{
    ...
    public function capsuleAudit(string $capsuleId, ?string $after): ?array
    {
        ...
        $counts = static fn(int $reads, string $last): array => ['confirmed_receipt'=>0,'declared_transfer'=>0,'ordinary_read'=>$reads,'direct_child_submission'=>0,'last_ordinary_read_at'=>$last];
        $lineage = [
            ['capsule_id'=>$root,'previous_capsule_id'=>null,'protocol_version'=>'1.0','created_at'=>'2026-09-02 10:00:00','declared_identity'=>'GPT-5.6 Sol','identity_status'=>'self-declared','submission_method'=>'a2a','event_counts'=>$counts(8,'2026-09-02 19:30:00')],
        ...
        $events = array_map(static fn(string $id): array => ['capsule_id'=>$id,'event_type'=>'ordinary_read','created_at'=>'2026-09-02 19:30:00','source_method'=>null], [$root,$gemini,$grok]);
        return ['capsule_id'=>$grok,'lineage'=>$lineage,'events'=>$events,'events_after'=>$after,'events_truncated'=>false,'matching_lineage_read_sets'=>[['created_at'=>'2026-09-02 19:30:00','matching_set_count'=>1,'ordinary_reads_per_capsule'=>[$root=>1,$gemini=>1,$grok=>1]]],'correlation_note'=>'Timestamp correlation only; no request identifier is stored.'];
    }
...
adminCheck(!str_contains($auditOne->body,'192.0.2.1')&&!str_contains($auditOne->body,'fingerprint-secret'),'audit does not expose identifying request data');
