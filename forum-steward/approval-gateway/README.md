...
Zalogowany administrator ma również pasywny audyt odczytów kapsuł pod `/panel/capsule-audit`. Audyt przyjmuje wyłącznie pełny `capsule_id`, podąża po bezpośrednich przodkach i wykonuje przygotowane zapytania `SELECT` w transakcji tylko do odczytu. Pokazuje typ i czas zdarzenia, zapisany `submission_method`, liczniki, `read_method` oraz `read_batch_id`. Sekcja „Zweryfikowane odczyty pełnego lineage” wymaga dokładnego wspólnego batcha, jednej metody lineage, pełnego zestawu kapsuł ancestry i liczby eventów równej długości lineage. Osobna sekcja historyczna obejmuje wyłącznie stare eventy bez obu metadanych i opisuje zgodność timestampu jako korelację, nie dowód żądania.

Audyt nie wywołuje publicznych endpointów kapsuł, nie tworzy zdarzeń, nie zmienia liczników i nie pokazuje adresów IP, User-Agentów, fingerprintów, tokenów, sekretów, cookies ani identyfikatorów sesji. Historyczne `confirmed_receipt` mogą pochodzić z okresu sprzed rozdzielenia potwierdzonego odbioru od tworzenia dzieci; panel opisuje tę możliwość jako „Legacy historical semantics” i nie zmienia żadnej liczby ani eventu.

Strona audytu korzysta z tej samej prywatnej sesji administratora, CSP, `no-store`, `noindex`, `SameSite=Strict`, `HttpOnly` i `Secure` co panel moderacji. Nie istnieje publiczny endpoint audytowy, katalog ani wyszukiwarka kapsuł; robots i sitemap publicznego HRM pozostają bez zmian.
