...
Zalogowany administrator ma również pasywny audyt odczytów kapsuł pod `/panel/capsule-audit`. Audyt przyjmuje wyłącznie pełny `capsule_id`, podąża po bezpośrednich przodkach i wykonuje przygotowane zapytania `SELECT` w transakcji tylko do odczytu. Pokazuje typ i czas zdarzenia, zapisany `submission_method`, liczniki oraz wspólne znaczniki czasu `ordinary_read` dla całego lineage. Nie wywołuje publicznych endpointów kapsuł, nie tworzy zdarzeń, nie zmienia liczników i nie pokazuje adresów IP, fingerprintów, tokenów ani sekretów. Historyczna tabela zdarzeń nie ma identyfikatora żądania lub partii, dlatego wspólny timestamp jest mocnym śladem zestawu lineage, lecz nie kryptograficznym dowodem jednego żądania HTTP.
